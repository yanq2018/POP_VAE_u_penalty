...
class POPVAE(nn.Module):
    ...
    def __init__(self, x_h, x_w, h_dim, z_dim, u_dim, mb_size, nc,device,num_parts,num_parts_h,num_parts_w,part_h, part_w,t,M):
        ...
        Place = Place.repeat(self.bsz,1,1)
        self.anchor_place = F.affine_grid(Place, torch.Size([Place.shape[0],1,self.h,self.w]))
        KM = F.grid_sample(M.view(-1,self.h,self.w).unsqueeze(1), self.anchor_place, padding_mode='zeros').view(-1,1,self.h,self.w)
        self.M = KM
    ...
    def forward(self, inputs):
        ...
        parts = self.z2parts(torch.ones(self.bsz,self.z_dim)).view(self.bsz,self.num_parts,self.part_dim)
        '''
        ...
        anchor_place = self.anchor_place[0:inputs.size()[0]*self.num_parts,:,:,:]
        ...
        #u = u_large_mu.view(-1,self.u1_dim)#u_large.view(-1,self.u1_dim)
        ...
        theta_g = rotTrMatrix(self.u2u(global_u),self.dv)
