...
def loss_V(recon_x, x, mu2, var2,glb):
    ...
    return BCE, KLD2+KLDb


def NLL_test_loss(recon_x, x, mu1, var1,mu2,var2,z,u,glb):
...
def test(epoch):
    ...
    with torch.no_grad():
        for _, (data, target) in enumerate(te):
            ...
            recon_batch, zmu, zvar, umu, uvar,z,u, glb,parts,KM= model(data)
            #NLL = NLL_test_loss(recon_batch, data, zmu, zvar, umu, uvar,z,u,glb)
            recon_loss, kl = loss_V(recon_batch, data, zmu, zvar, umu, uvar,glb)
            test_recon_loss += recon_loss.item()
            
    test_recon_loss /= (len(te)*mb_size)
    print('====> Epoch:{} recon loss: {:.4f}'.format(epoch, test_recon_loss))
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(tr):
        ...
        recon_loss, kl = loss_V(recon_batch, data, umu, uvar,glb)
        
        loss = recon_loss + kl + 4*torch.mean(torch.norm(umu,2,dim=0)) 
        loss.backward()
...
for epoch in range(epochs):
    ...
    #if epoch==49:
    #    test(epoch)
torch.save(model.state_dict(), './ModDict/n'+str(clnum)+'control_shift2.model')
