...
def draw(idx):
    ...
    plt.imshow(recon_batch[idx,0,:,:].detach().numpy())
    ...
    for i in range(16):
        ...
        plt.imshow(parts[idx,i,:,:].detach().numpy()) #put your numpy array here
    plt.show()
    ...
    for i in range(16):
        fig.add_subplot(4,4,i+1)
        plt.imshow(KM[idx,i,:,:].detach().numpy()) #put your numpy array here
    plt.show()
    ...
    for i in range(16):
        fig.add_subplot(4,4,i+1)
        plt.imshow(x[idx,i,:,:].detach().numpy()) #put your numpy array here
    plt.show()
    
    fig,ax = plt.subplots(1)
    ax.imshow(p[idx,:,:].detach().numpy())
    for i in range(16):
...
def generatePart(z):
    ...
    for i in range(z.shape[0]):
        plt.imshow(interRecon[i,:,:].detach().numpy())
